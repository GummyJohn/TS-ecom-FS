...
import { Products, getData } from '../utils';
...
const Search = ({addCart}: SearchProps) => {
  ...
  useEffect(() => {
    getData(`http://localhost:4000/products/all`,
  ...
  return (
    <>
      <div 
        className='mt-20 h-[30vh] relative flex justify-center items-center flex-col'
      >
        <div className='flex absolute w-full h-full'>
          <motion.div 
            ...
            style={womenClothesBgImg} className='w-[25%] h-full'>
          </motion.div>
          <motion.div 
            initial={{x: '-100vw'}}
            animate={{x: 0}}
            transition={{duration: 1.8}}
            style={menClothesBgImg} className='w-[25%] h-full'>  
          </motion.div>
          <motion.div 
            initial={{x: '-100vw'}}
            animate={{x: 0}}
            transition={{duration: 1.5}}
            style={technologyBgImg} className='w-[25%] h-full'>  
          </motion.div>
          <motion.div style={jewelryBgImg} 
            initial={{x: '-100vw'}}
            animate={{x: 0}}
      ...
      <div>
        <div className='flex w-full justify-evenly items-center mt-10 '>
          ...
          className='text-center'>
            <button style={womenClothesBgImg}
